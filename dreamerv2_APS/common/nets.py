...
class EnsembleRSSM(common.Module):
    ...
    def _suff_stats_ensemble(self, inp):
        ...
        stats = {k: tf.stack([x[k] for x in stats], 0)
...
class Cluster(nn.Module):
    ...
    def __init__(self, config, embed_size):
        ...
        self.comp_mean = nn.Parameter(
            torch.randn(self.n_mix, self.z_dim) * np.sqrt(1.0 / self.n_mix)
        )

        self.comp_logstd = nn.Parameter(
            torch.randn(1, self.z_dim) * 1 / np.e, requires_grad=True
        )

        self.mix_logit = nn.Parameter(
            torch.ones(self.n_mix), requires_grad=config.cluster["learned_prior"]
        )
    ...
    def component_log_prob(self, x):
        ...
        comp_logstd = torch.clamp(
            self.comp_logstd,
            self.config.cluster["LOG_STD_MIN"],
            self.config.cluster["LOG_STD_MAX"],
        )
    ...
    def sample(self, candidate_num, sample_num):
        ...
        min_prob_indices = torch.topk(
            log_prob_points_sampled, sample_num, largest=False
        ).indices
    ...
    def forward(self, x, with_elbo=True):
        ...
        if with_elbo:
            kl_from_prior = kl_divergence(
                Categorical(probs=posterior), Categorical(probs=prior_prob)
            )
    ...
    def circles(self):
        with torch.no_grad():
            return (
                torch.exp(self.comp_logstd).clone().expand_as(
                    self.comp_mean).detach()
            )
    ...
    def assign_centroids(self, x):
        ...
        self.mix_logit = nn.Parameter(
            torch.ones(self.n_mix), requires_grad=self.config.cluster["learned_prior"]
        )
